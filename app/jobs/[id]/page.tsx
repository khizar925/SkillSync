                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6 md:p-8 max-w-md w-full">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                            ...
                            <button onClick={() => refetch()} className="text-sm text-emerald-600 hover:text-emerald-700 underline font-medium">
                                Try again
                            </button>
                        </div>
                    </div>
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 pt-20 md:pt-24">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6 md:p-8 mb-6">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">{jobData.job_title}</h1>
                    <p className="text-lg text-gray-700 font-medium mb-6">Company: {jobData.company_name}</p>
                    <div className="grid sm:grid-cols-2 gap-4 pt-6 border-t border-gray-200">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-medium text-gray-900">{jobData.job_location}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-gray-100 rounded-lg"><Clock className="h-5 w-5 text-gray-600" /></div>
                            <div>
                                <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold mb-1">Employment Type</p>
                                <p className="text-sm font-medium text-gray-900">{formatEmploymentType(jobData.employment_type as EmploymentType)}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex border-b border-gray-200">
                        {(['overview', 'application'] as const).map(tab => (
                            <button
                                ...
                                className={`flex-1 py-4 text-center text-sm font-semibold transition-colors duration-200 capitalize ${activeTab === tab ? 'text-emerald-600 border-b-2 border-emerald-600' : 'text-gray-500 hover:text-gray-700'}`}
                            >
                                {tab}
                            </button>
                        ))}
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 p-6 md:p-8 mb-6">
                        {hasApplied ? (
                            <div className="text-center py-12">
                                ...
                                <h3 className="text-2xl font-bold text-gray-900 mb-2">Application Already Submitted</h3>
                                <p className="text-gray-600 mb-8 max-w-md mx-auto">
                                    You have already applied for this position. Your current application status is:
                                    <span className={`block mt-2 text-lg font-bold uppercase tracking-wider ${applicationStatus === 'rejected' ? 'text-red-600' : applicationStatus === 'shortlisted' ? 'text-green-600' : 'text-emerald-600'}`}>
                                        {applicationStatus}
                                    </span>
                                </p>
                            ...
                            <>
                                ...
                                <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    {/* Resume */}
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-900 mb-2">Resume/CV *</label>
                                        <div className="space-y-2">
                                            <label htmlFor="resume"
                                                className={`flex items-center justify-center gap-2 w-full px-4 py-3 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${formErrors.resume ? 'border-red-300 bg-red-50 hover:bg-red-100' : 'border-gray-300 bg-gray-50 hover:bg-gray-100'}`}>
                                            ...
                                            {formData.resume && (
                                                <div className="flex items-center gap-2 text-sm text-gray-600">
                                                    ...
                                                    <span className="text-gray-400">({(formData.resume.size / 1024 / 1024).toFixed(2)} MB)</span>
                                                </div>
                                            )}
                    ...
                    <motion.div initial={{ opacity: 0, y: 50, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                        ...
                        <div className={`p-4 rounded-xl shadow-2xl border flex items-start gap-3 bg-white ${submitMessage.type === 'success' ? 'border-green-100' : 'border-red-100'}`}>
                            ...
                            <div className="flex-1 pt-0.5">
                                ...
                                <p className="text-sm text-gray-600 mt-1">{submitMessage.text}</p>
                            </div>
                            <button onClick={() => setSubmitMessage(null)} className="p-1 hover:bg-gray-100 rounded-md transition-colors">
